...
window.handleGoogleLogin = handleGoogleLogin;
window.handleNewGoogleUser = handleNewGoogleUser;
window.showUsernameModalWithFallback = showUsernameModalWithFallback;
window.fallbackDirectLogin = fallbackDirectLogin;
...
async function handleGoogleLogin() {
    ...
    try {
        ...
        // CORREZIONE 1: Timeout per evitare blocchi infiniti
        const loginPromise = signInWithPopup(window.firebaseAuth, window.googleProvider);
        const timeoutPromise = new Promise((_, reject) => 
            setTimeout(() => reject(new Error('Timeout login Google')), 30000)
        );
        ...
        // CORREZIONE 2: Gestione più robusta del controllo utente esistente
        let userData = null;
        ...
        try {
            ...
            if (snapshot.exists()) {
                ...
                console.log('🆕 Nuovo utente Google');
            }
        } catch (dbError) {
            console.error('⚠️ Errore lettura database:', dbError);
            // Continua comunque con nuovo utente
        }

        // CORREZIONE 3: Gestione più chiara del flusso username
        if (!isExistingUser) {
            console.log('🆕 Nuovo utente - preparazione dati...');
            await handleNewGoogleUser(user);
        } else if (userData.needsUsername === true || !userData.username || userData.username.trim() === '') {
            console.log('⚠️ Utente senza username valido, mostrando modal...');
            await handleGoogleUserNeedsUsername(user, userData);
        } else {
        ...
        // CORREZIONE 4: Ripristino più robusto del pulsante
        setTimeout(() => {
            if (googleBtn) {
...
async function handleNewGoogleUser(user) {
    try {
        ...
        // Mostra modal username con fallback robusto
        await showUsernameModalWithFallback(user, userData);
        
    } catch (error) {
        console.error('❌ Errore gestione nuovo utente:', error);
        // Fallback: continua login senza username personalizzato
        await fallbackDirectLogin(user);
    }
}

// CORREZIONE 6: Gestione utenti esistenti senza username
async function handleGoogleUserNeedsUsername(user, userData) {
    try {
        console.log('⚠️ Utente esistente senza username:', user.email);
        await showUsernameModalWithFallback(user, userData);
    } catch (error) {
        console.error('❌ Errore gestione username esistente:', error);
        await fallbackDirectLogin(user);
    }
}

// CORREZIONE 7: Modal username con fallback robusto
async function showUsernameModalWithFallback(user, userData = null) {
    const maxAttempts = 3;
    let attempt = 0;
    ...
    while (attempt < maxAttempts) {
        try {
            attempt++;
            console.log(`🎯 Tentativo ${attempt}/${maxAttempts} - Mostrando modal username`);
            
            if (window.usernameManager && typeof window.usernameManager.showUsernameModal === 'function') {
                await window.usernameManager.showUsernameModal(user, userData);
                return; // Successo
            } else {
                throw new Error('usernameManager non disponibile');
            }
            
        } catch (error) {
            console.warn(`⚠️ Tentativo ${attempt} fallito:`, error);
    ...
    console.log('🚨 Tutti i tentativi modal falliti, usando fallback');
    await fallbackDirectLogin(user);
}

// CORREZIONE 8: Fallback per login diretto
async function fallbackDirectLogin(user) {
    console.log('🔄 Fallback: login diretto senza username personalizzato');
    
    try {
        // Genera username di backup dall'email
        const backupUsername = user.email.split('@')[0].replace(/[^a-zA-Z0-9]/g, '') + '_user';
        
        // Aggiorna i dati utente con username di backup
        const userRef = ref(window.firebaseDatabase, `users/${user.uid}`);
        ...
        try {
            ...
            // Aggiorna con username di backup
            userData.username = backupUsername;
            userData.needsUsername = false;
            userData.lastSeen = Date.now();
            
            await set(userRef, userData);
            console.log('✅ Username di backup salvato:', backupUsername);
            
        } catch (updateError) {
            console.error('⚠️ Errore aggiornamento dati (non critico):', updateError);
        }
        
        // Aggiorna displayName se possibile
        try {
            if (updateProfile) {
                await updateProfile(user, { displayName: backupUsername });
            }
        } catch (profileError) {
        ...
        // Completa il login
        console.log('✅ Completando login con fallback');
        completeUserLogin(user);
        
        // Notifica all'utente
        setTimeout(() => {
            alert(`✅ Login completato!\nUsername temporaneo: ${backupUsername}\n\nPuoi personalizzarlo in seguito nelle impostazioni.`);
        }, 1000);
        
    } catch (error) {
        console.error('❌ Errore anche nel fallback:', error);
        
        // Ultimo tentativo: login base senza personalizzazioni
        try {
            completeUserLogin(user);
            setTimeout(() => {
                alert('✅ Login completato in modalità base.\nAlcune funzionalità potrebbero essere limitate.');
            }, 1000);
        } catch (finalError) {
            console.error('❌ Errore critico nel login:', finalError);
            showError('Errore critico nel login. Riprova o contatta il supporto.');
        }
    }
}

// CORREZIONE 9: Gestione errori più specifica
async function handleGoogleLoginError(error) {
    ...
    switch (error.code) {
        ...
        case 'auth/app-check-token-invalid':
            errorMessage = 'Errore di sicurezza. Ricarica la pagina e riprova.';
            shouldRetry = true;
            break;
        default:
            if (error.message) {
                if (error.message.includes('Timeout')) {
                    errorMessage = 'Timeout di connessione. Riprova.';
                    shouldRetry = true;
                } else {
    ...
    // Opzione di retry per alcuni errori
    if (shouldRetry) {
        setTimeout(() => {
...
// CORREZIONE 10: Gestione onAuthStateChanged più robusta
function setupAuthStateListener() {
    if (!window.firebaseAuth || !onAuthStateChanged) return;
    
    onAuthStateChanged(window.firebaseAuth, async (user) => {
        try {
            if (user) {
                console.log('🔄 Stato auth cambiato - utente loggato:', user.email);
                currentUser = user;
                
                // Verifica se questo è un login Google in corso
                if (window.usernameManager && window.usernameManager.pendingUser && 
                    window.usernameManager.pendingUser.uid === user.uid) {
                    console.log('⏳ Login Google in corso, aspettando completamento username...');
                    return; // Non procedere, lascia che il flusso username si completi
                }
                
                // Login normale o completato
                await handleUserLogin(user);
            } else {
                console.log('🔄 Stato auth cambiato - utente disconnesso');
                currentUser = null;
                handleUserLogout();
            }
        } catch (error) {
...
window.monitorDataUsage = function() {
    if (!window.useFirebase) return;
    
    let totalReads = 0;
    let totalWrites = 0;
    
    // Intercetta get/onValue originali
    const originalGet = window.firebaseImports.get;
    const originalOnValue = window.firebaseImports.onValue;
    
    window.firebaseImports.get = function(...args) {
        totalReads++;
        console.log(`📊 Read #${totalReads}`, args[0]._path);
        return originalGet.apply(this, args);
    };
    
    window.firebaseImports.onValue = function(...args) {
        totalReads++;
        console.log(`📊 Listener #${totalReads}`, args[0]._path);
        return originalOnValue.apply(this, args);
    };
    
    // Report ogni minuto
    setInterval(() => {
        console.log(`📈 Consumo dati: ${totalReads} letture, ${totalWrites} scritture`);
    }, 60000);
};

// Attiva monitoring
window.monitorDataUsage();