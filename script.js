...
window.handleEmailPasswordRegistration = async function(email, password, username) {
    try {
        console.log('📝 Registrazione email/password:', { email, username });
        ...
        console.log('✅ Account creato:', user.uid);
        
        // Aggiorna il displayName con l'username scelto
        await updateProfile(user, {
            displayName: username
        });
        
        // Salva i dati dell'utente usando il username manager
        await window.usernameManager.saveEmailPasswordUser(user, username, email);
        
        console.log('✅ Utente email/password completamente configurato');
        
        return user;
        
    } catch (error) {
        console.error('❌ Errore registrazione email/password:', error);
        throw error;
    }
};

// Modifica la funzione handleSubmit esistente
window.handleSubmit = async function() {
    ...
    if (!isLogin && !username) {
        showLoginError('Username è obbligatorio per la registrazione');
        return;
    }
    ...
    try {
        ...
        if (isLogin) {
            ...
            // L'utente verrà gestito da onAuthStateChanged
            
        } else {
            // REGISTRAZIONE CON EMAIL/PASSWORD
            console.log('📝 Tentativo registrazione con email/password');
            const user = await window.handleEmailPasswordRegistration(email, password, username);
            console.log('✅ Registrazione riuscita per:', user.email);
...
// Migliora la gestione di onAuthStateChanged
window.setupAuthStateListener = function() {
    ...
    onAuthStateChanged(window.firebaseAuth, async (user) => {
        ...
        if (user) {
            ...
            try {
                // Controlla se l'utente ha bisogno di scegliere un username
                const needsUsername = await window.usernameManager.checkUserNeedsUsername(user);
                
                if (needsUsername) {
                    console.log('🎯 Utente ha bisogno di scegliere username');
                    // Mostra il modal per scegliere username (solo per utenti Google)
                    await window.usernameManager.showUsernameModal(user);
                } else {
                    console.log('✅ Utente ha già un username, procedo con il login');
                    // Completa il login normalmente
                    if (typeof window.completeUserLogin === 'function') {
                        window.completeUserLogin(user);
                    } else if (typeof window.handleUserLogin === 'function') {
                        window.handleUserLogin(user);
                    } else {
                        console.warn('⚠️ Nessuna funzione di login trovata');
                    }
                }
            } catch (error) {
                console.error('❌ Errore durante controllo username:', error);
                // In caso di errore, procedi comunque con il login
                if (typeof window.completeUserLogin === 'function') {
                    window.completeUserLogin(user);
                } else if (typeof window.handleUserLogin === 'function') {
                    window.handleUserLogin(user);
...
// Funzioni di utilità per il form
window.switchToRegister = function() {
    ...
    if (googleBtnText) {
        googleBtnText.textContent = 'Registrati con Google';
    }
...
window.switchToLogin = function() {
    ...
    if (googleBtnText) {
        googleBtnText.textContent = 'Continua con Google';
    }
...
// Funzione per pulire i messaggi di errore/successo
window.clearLoginMessages = function() {
    const errorEl = document.getElementById('loginError');
    const successEl = document.getElementById('loginSuccess');
...
// ===============================================
// ASSICURATI CHE QUESTE FUNZIONI ESISTANO GIÀ
// ===============================================

// Se non esistono già, aggiungi anche queste:
...
window.showLoading = function(show) {
    ...
    if (submitBtn) {
        ...
        submitBtn.textContent = show ? 'Attendere...' : (document.getElementById('loginTab').classList.contains('active') ? 'Accedi' : 'Registrati');
    }
};